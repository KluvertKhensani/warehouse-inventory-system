...
drop function if exists public.list_putaway_operators();

create function public.list_putaway_operators()
returns table (
  profile_id uuid,
  full_name text,
  email text,
...
begin
  ...
  if authenticated_user_id is null then
    raise exception
      'Authentication is required to load putaway operators.';
  end if;
...
-- List putaway operators function permissions

revoke all
on function public.list_putaway_operators()
from public;
